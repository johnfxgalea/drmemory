...
#define APP_RANGE_LOOP(do_once, app_addr, app_size, app_blk_base, app_blk_end, app_src_end, \
                       start, end, iter_size, loop_body) do {                      \
                                                                                   \
    app_src_end = app_addr + (app_size - 1); /* closed, to avoid overflow */       \
    do_once = (app_addr == (app_pc) ALIGN_BACKWARD(app_addr, map->app_block_size)) && \
        (app_src_end == (app_pc) ALIGN_BACKWARD(app_addr, map->app_block_size)); \
    for (app_blk_base = (app_pc) ALIGN_BACKWARD(app_addr, map->app_block_size);    \
          ...
          */                                                                       \
        do_once || app_blk_base < app_src_end;                                               \
