...
ptr_uint_t umbra_map_scale_app_to_shadow(umbra_map_t *map, ptr_uint_t value) {
...
ptr_uint_t umbra_map_scale_shadow_to_app(umbra_map_t *map, ptr_uint_t value) {
    if (UMBRA_MAP_SCALE_IS_DOWN(map->options.scale))
        value <<= map->shift;
    else if (UMBRA_MAP_SCALE_IS_UP(map->options.scale))
        value >>= map->shift;
...
void umbra_map_lock(umbra_map_t *map) {
    dr_recurlock_lock(map->lock);
}

void umbra_map_unlock(umbra_map_t *map) {
    dr_recurlock_unlock(map->lock);
}

static void umbra_map_destroy(umbra_map_t *map) {
    if (map == NULL)
        return;
...
static bool umbra_map_compatible(umbra_map_t *map1, umbra_map_t *map2) {
...
static drmf_status_t umbra_map_create(umbra_map_t **map_out,
        umbra_map_options_t *ops, uint idx) {
    umbra_map_t *map;
...
static drmf_status_t umbra_map_add(umbra_map_t **map_out,
    ...
    if (res != DRMF_SUCCESS)
        return res;ASSERT(*map_out != NULL, "fail to create umbra map");
    res = DRMF_SUCCESS;
    /* check compatibility */
...
static void umbra_map_remove(umbra_map_t *map) {
    uint i;
...
static bool umbra_event_filter_syscall(void *drcontext, int sysnum) {
    /* NYI */
    return false;
}

static bool umbra_event_pre_syscall(void *drcontext, int sysnum) {
    /* NYI */
    return true;
}

static void umbra_event_post_syscall(void *drcontext, int sysnum) {
    /* NYI */
}
...
#ifdef UNIX
dr_signal_action_t umbra_event_signal(void *drcontext, dr_siginfo_t *info) {
...
drmf_status_t umbra_init(client_id_t client_id) {
    drmf_status_t res;
    drmgr_priority_t pri_excpt = { sizeof(pri_excpt),
    DRMGR_PRIORITY_NAME_EXCPT_UMBRA, NULL, NULL, DRMGR_PRIORITY_EXCPT_UMBRA };
 ...
 */
DR_EXPORT
drmf_status_t umbra_create_shadow_memory(IN umbra_map_t *map, IN uint flags,
    ...
    if (app_size == 0)
        return DRMF_SUCCESS;
    return umbra_create_shadow_memory_arch(map, flags, app_addr, app_size,
            value, value_size);
}
...
DR_EXPORT
drmf_status_t umbra_insert_app_to_shadow(IN void *drcontext,
        IN umbra_map_t *map, IN instrlist_t *ilist, IN instr_t *where,
    ...
    return umbra_insert_app_to_shadow_arch(drcontext, map, ilist, where,
            addr_reg, scratch_regs, num_scratch_regs);
}

DR_EXPORT
drmf_status_t umbra_read_shadow_memory(IN umbra_map_t *map, IN app_pc app_addr,
        IN size_t app_size, OUT size_t *shadow_size, IN byte *buffer) {
    if (map == NULL || map->magic != UMBRA_MAP_MAGIC) {
        ASSERT(false, "invalid umbra_map");
        return DRMF_ERROR_INVALID_PARAMETER;
    }
    ...
    return umbra_read_shadow_memory_arch(map, app_addr, app_size, shadow_size,
            buffer);
}
...
drmf_status_t umbra_write_shadow_memory(IN umbra_map_t *map, IN app_pc app_addr,
    ...
    return umbra_write_shadow_memory_arch(map, app_addr, app_size, shadow_size,
            buffer);
}

DR_EXPORT
drmf_status_t umbra_shadow_set_range(IN umbra_map_t *map, IN app_pc app_addr,
        IN size_t app_size, OUT size_t *shadow_size, IN ptr_uint_t value,
        IN size_t value_size) {
    if (map == NULL || map->magic != UMBRA_MAP_MAGIC) {
        ASSERT(false, "invalid umbra_map");
        return DRMF_ERROR_INVALID_PARAMETER;
...
drmf_status_t umbra_shadow_copy_range(IN umbra_map_t *map, IN app_pc app_src,
        IN app_pc app_dst, IN size_t app_size, OUT size_t *shadow_size) {
    if (map == NULL || map->magic != UMBRA_MAP_MAGIC) {
        ASSERT(false, "invalid umbra_map");
        return DRMF_ERROR_INVALID_PARAMETER;
    }
    ...
    return umbra_shadow_copy_range_arch(map, app_src, app_dst, app_size,
            shadow_size);
}

DR_EXPORT
drmf_status_t umbra_value_in_shadow_memory(IN umbra_map_t *map,
    ...
    return umbra_value_in_shadow_memory_arch(map, app_addr, app_size, value,
            value_size, found);
}

DR_EXPORT
drmf_status_t umbra_get_shadow_block_size(IN umbra_map_t *map, OUT size_t *size) {
    if (map == NULL || map->magic != UMBRA_MAP_MAGIC) {
        ASSERT(false, "invalid umbra_map");
        return DRMF_ERROR_INVALID_PARAMETER;
    }
...
drmf_status_t umbra_iterate_app_memory(IN umbra_map_t *map, IN void *user_data,
...
DR_EXPORT
drmf_status_t umbra_iterate_shadow_memory(umbra_map_t *map, void *user_data,
...
DR_EXPORT
drmf_status_t umbra_get_shadow_memory_type(IN umbra_map_t *map,
...
drmf_status_t umbra_shadow_memory_is_shared(IN umbra_map_t *map,
        IN byte *shadow_addr, OUT umbra_shadow_memory_type_t *shadow_type) {
    if (map == NULL || map->magic != UMBRA_MAP_MAGIC) {
        ASSERT(false, "invalid umbra_map");
        return DRMF_ERROR_INVALID_PARAMETER;
...
DR_EXPORT
drmf_status_t umbra_get_shadow_memory(IN umbra_map_t *map, IN app_pc app_addr,
        OUT byte **shadow_addr, INOUT umbra_shadow_memory_info_t *shadow_info) {
    if (map == NULL || map->magic != UMBRA_MAP_MAGIC) {
        ASSERT(false, "invalid umbra_map");
        return DRMF_ERROR_INVALID_PARAMETER;
    }
    ...
    if (shadow_info != NULL && shadow_info->struct_size != sizeof(*shadow_info))
        return DRMF_ERROR_INVALID_PARAMETER;
    return umbra_get_shadow_memory_arch(map, app_addr, shadow_addr, shadow_info);
}

DR_EXPORT
drmf_status_t umbra_replace_shared_shadow_memory(umbra_map_t *map,
