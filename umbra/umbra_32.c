...
static byte *shadow_table_init_redzone(umbra_map_t *map, byte *block) {
  ...
  if (map->options.redzone_size != 0) {
    ...
    if (map->options.make_redzone_faulty) {
      ...
      IF_DEBUG(ok =)
      dr_memory_protect(block + map->shadow_block_size, map->shadow_block_size,
                        DR_MEMPROT_NONE);
      ASSERT(ok, "-w failed: will have inconsistencies in shadow data");
...
int umbra_num_scratch_regs_for_translation_arch(void) { return 1; }
...
shadow_table_insert_app_to_shadow_arch(void *drcontext, umbra_map_t *map,
  ...
  PRE(ilist, where,
      INSTR_CREATE_add(drcontext, opnd_create_reg(reg_addr),
                       opnd_create_base_disp(REG_NULL, reg_idx,
                                             sizeof(ptr_int_t), disp,
                                             OPSZ_PTR)));
}
#elif defined(ARM)
/* code sequence:
...
static bool shadow_table_is_in_normal_block(umbra_map_t *map,
                                            byte *shadow_addr) {
  return (
      !shadow_table_is_in_default_block(map, shadow_addr, NULL) &&
...
static void shadow_table_init(umbra_map_t *map) {
  ...
  if (!TEST(UMBRA_MAP_CREATE_SHADOW_ON_TOUCH, map->options.flags)) {
    ...
    byte *start = shadow_table_create_special_block(
...
drmf_status_t umbra_arch_init() { return DRMF_SUCCESS; }
...
void umbra_map_arch_exit(umbra_map_t *map) { shadow_table_exit(map); }

drmf_status_t umbra_create_shadow_memory_arch(umbra_map_t *map, uint flags,
                                              app_pc app_addr, size_t app_size,
                                              ptr_uint_t value,
                                              size_t value_size) {
  ...
  if (POINTER_OVERFLOW_ON_ADD(app_addr,
                              app_size - 1)) /* just hitting top is ok */
...
drmf_status_t umbra_write_shadow_memory_arch(IN umbra_map_t *map,
  ...
  APP_RANGE_LOOP(
      ...
      iter_size, {
        ...
        if (shadow_table_is_in_default_block(map, shadow_start, NULL))
          return DRMF_ERROR_INVALID_PARAMETER;
        if (shadow_table_is_in_special_block(map, shadow_start, NULL, NULL,
                                             NULL)) {
          shadow_table_replace_block(map, app_blk_base);
          shadow_start = shadow_table_app_to_shadow(map, start);
        }
...
drmf_status_t
umbra_shadow_set_range_arch(IN umbra_map_t *map, IN app_pc app_addr,
...
drmf_status_t umbra_shadow_copy_range_arch(IN umbra_map_t *map,
                                           IN app_pc app_src, IN app_pc app_dst,
  ...
  tot_shadow_sz = 0;
  APP_RANGE_LOOP(
      app_src, app_sz, app_blk_base, app_blk_end, app_src_end, start, end,
      iter_size, {
        shadow_start = shadow_table_app_to_shadow(map, start);
        if (shadow_table_is_in_default_block(map, shadow_start, NULL)) {
        ...
        res = umbra_write_shadow_memory_arch(map, app_dst, iter_size,
                                             &shadow_sz, shadow_start);
        if (res != DRMF_SUCCESS) {
  ...
  APP_RANGE_LOOP(
      *app_addr, app_size, app_blk_base, app_blk_end, app_src_end, start, end,
      iter_size, {
        ...
        if (shadow_addr != NULL) {
          *app_addr = start + umbra_map_scale_shadow_to_app(
                                  map, shadow_addr - shadow_start);
          *found = true;
          return DRMF_SUCCESS;
        }
      });
...
drmf_status_t umbra_insert_app_to_shadow_arch(void *drcontext, umbra_map_t *map,
                                              instrlist_t *ilist,
                                              instr_t *where, reg_id_t addr_reg,
...
drmf_status_t umbra_shadow_memory_is_shared_arch(
    IN umbra_map_t *map, IN byte *shadow_addr,
...
drmf_status_t
umbra_get_shadow_memory_arch(umbra_map_t *map, app_pc app_addr,
                             byte **shadow_addr,
  ...
  if (shadow_info != NULL) {
    shadow_info->app_base =
        (app_pc)ALIGN_BACKWARD(app_addr, map->app_block_size);
