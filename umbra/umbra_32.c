...
static inline ptr_uint_t shadow_table_get_block_offset(umbra_map_t *map,
        app_pc app_addr) {
    ptr_uint_t offset = SHADOW_TABLE_OFFSET(app_addr);
    return umbra_map_scale_app_to_shadow(map, offset);
}

static void shadow_table_delete_block(umbra_map_t *map, byte *shadow_start) {
...
static byte *shadow_table_init_redzone(umbra_map_t *map, byte *block) {
    ...
    if (map->options.redzone_size != 0) {
        ...
        if (map->options.make_redzone_faulty) {
            ...
            memset(block, map->options.redzone_value,
 ...
 */
static void shadow_table_delete_special_block(umbra_map_t *map,
...
static void shadow_table_create_special_block_helper(umbra_map_t *map,
        ptr_uint_t value, size_t value_size, special_block_t *special_block) {
    IF_DEBUG(bool ok;)
    byte *block;
...
static void shadow_table_create_default_block(umbra_map_t *map) {
...
static void shadow_table_set_block(umbra_map_t *map, uint idx, byte *block) {
...
int umbra_num_scratch_regs_for_translation_arch(void) {
    return 1;
}
 ...
 */
static void shadow_table_insert_app_to_shadow_arch(void *drcontext,
        umbra_map_t *map, instrlist_t *ilist, instr_t *where, reg_id_t reg_addr,
        reg_id_t reg_idx) {
    uint disp;
    ...
    PRE(ilist, where,
            INSTR_CREATE_add(drcontext, opnd_create_reg(reg_addr),
...
static bool shadow_table_is_in_default_block(umbra_map_t *map,
        byte *shadow_addr, bool *redzone OUT) {
    if (shadow_addr >= map->default_block.start - map->options.redzone_size
            && shadow_addr
                    < (map->default_block.start + map->shadow_block_size
                            + map->options.redzone_size)) {
        if (redzone != NULL) {
            if (map->options.redzone_size != 0
                    && (shadow_addr < map->default_block.start
                            || shadow_addr
                                    >= (map->default_block.start
                                            + map->shadow_block_size)))
                *redzone = true;
            else
                *redzone = false;
...
static bool shadow_table_use_default_block(umbra_map_t *map, app_pc app_addr) {
    return shadow_table_is_in_default_block(map,
            shadow_table_app_to_shadow(map, app_addr), NULL);
}

static bool shadow_table_is_in_special_block(umbra_map_t *map,
        byte *shadow_addr, ptr_uint_t *value, size_t *value_size,
        bool *redzone OUT) {
    uint i;
...
static bool shadow_table_use_special_block(umbra_map_t *map, app_pc app_addr,
        ptr_uint_t *value, size_t *value_size) {
    return shadow_table_is_in_special_block(map,
            shadow_table_app_to_shadow(map, app_addr), value, value_size, NULL);
}
...
static void shadow_table_replace_block(umbra_map_t *map, app_pc app_base) {
    ...
    if (shadow_table_use_default_block(map, app_base)
            || shadow_table_use_special_block(map, app_base, &value,
                    &value_size)) {
...
static void shadow_table_init(umbra_map_t *map) {
    uint i;
    ...
    if (!TEST(UMBRA_MAP_CREATE_SHADOW_ON_TOUCH, map->options.flags)) {
        ...
        byte *start = shadow_table_create_special_block(map,
...
static void shadow_table_exit(umbra_map_t *map) {
    uint i;
...
drmf_status_t umbra_arch_init() {
    return DRMF_SUCCESS;
}

void umbra_arch_exit() {
}

drmf_status_t umbra_map_arch_init(umbra_map_t *map, umbra_map_options_t *ops) {
    ...
    /* Create a shadow block surround by 2 redzones */
    map->shadow_block_alloc_size = map->shadow_block_size
            + 2 * map->options.redzone_size;

    shadow_table_init(map);
    return DRMF_SUCCESS;
}

void umbra_map_arch_exit(umbra_map_t *map) {
    shadow_table_exit(map);
}

drmf_status_t umbra_create_shadow_memory_arch(umbra_map_t *map, uint flags,
        app_pc app_addr, size_t app_size, ptr_uint_t value, size_t value_size) {
    ...
    APP_RANGE_LOOP(app_addr, app_size, app_blk_base, app_blk_end, app_src_end,
            ...
            if (TEST(flags, UMBRA_CREATE_SHADOW_SHARED_READONLY) && ((app_blk_base >= app_addr && app_blk_end <= app_src_end) || (map->options.default_value == value && map->options.default_value_size == value_size))) {
            /* We can use a special block if either it is a whole block
             * or the block value is the same as the default value.
             */
            shadow_blk = shadow_table_create_special_block(map, value, value_size); if (shadow_blk != NULL) { shadow_table_set_block(map, SHADOW_TABLE_INDEX(app_blk_base), shadow_blk); continue; } }
            /* cannot use a special block, need create normal block */
            shadow_table_replace_block(map, app_blk_base); } res = umbra_shadow_set_range_arch(map, start, iter_size, &size, value, value_size); if (res != DRMF_SUCCESS) {
             ...
             */
            umbra_map_unlock(map); return res; } });
    umbra_map_unlock(map);
    return DRMF_SUCCESS;
}

drmf_status_t umbra_delete_shadow_memory_arch(umbra_map_t *map, app_pc app_addr,
        size_t app_size) {
    size_t shadow_size;
...
drmf_status_t umbra_read_shadow_memory_arch(IN umbra_map_t *map,
        IN app_pc app_addr, IN size_t app_size, INOUT size_t *shadow_size,
        IN byte *buffer) {
    /* i#1260: end pointers are all closed (i.e., inclusive) to handle overflow */
    app_pc app_blk_base, app_blk_end, app_src_end;
    app_pc start, end;
    ...
    APP_RANGE_LOOP(app_addr, app_size, app_blk_base, app_blk_end, app_src_end,
            start, end, iter_size,
            { shadow_start = shadow_table_app_to_shadow(map, start); if (shadow_table_is_in_default_block(map, shadow_start, NULL)) return DRMF_ERROR_INVALID_PARAMETER; size = umbra_map_scale_app_to_shadow(map, iter_size); memcpy(buffer, shadow_start, size); shdw_size += size; buffer += size; });
    *shadow_size = shdw_size;
    return DRMF_SUCCESS;
}

drmf_status_t umbra_write_shadow_memory_arch(IN umbra_map_t *map,
        IN app_pc app_addr, IN size_t app_size, INOUT size_t *shadow_size,
        IN byte *buffer) {
    /* i#1260: end pointers are all closed (i.e., inclusive) to handle overflow */
    app_pc app_blk_base, app_blk_end, app_src_end;
    ...
    APP_RANGE_LOOP(app_addr, app_size, app_blk_base, app_blk_end, app_src_end,
            start, end, iter_size,
            { shadow_start = shadow_table_app_to_shadow(map, start); if (shadow_table_is_in_default_block(map, shadow_start, NULL)) return DRMF_ERROR_INVALID_PARAMETER; if (shadow_table_is_in_special_block(map, shadow_start, NULL, NULL, NULL)) { shadow_table_replace_block(map, app_blk_base); shadow_start = shadow_table_app_to_shadow(map, start); } size = umbra_map_scale_app_to_shadow(map, iter_size); memmove(shadow_start, buffer, size); shdw_size += size; buffer += size; });
    *shadow_size = shdw_size;
    return DRMF_SUCCESS;
}

drmf_status_t umbra_shadow_set_range_arch(IN umbra_map_t *map,
        IN app_pc app_addr, IN size_t app_size, OUT size_t *shadow_size,
    ...
    APP_RANGE_LOOP(app_addr, app_size, app_blk_base, app_blk_end, app_src_end,
            start, end, iter_size,
            { shadow_start = shadow_table_app_to_shadow(map, start); if (shadow_table_is_in_default_block(map, shadow_start, NULL)) return DRMF_ERROR_INVALID_PARAMETER; size = umbra_map_scale_app_to_shadow(map, iter_size); if (shadow_table_is_in_special_block(map, shadow_start, &blk_val, &blk_val_sz, NULL)) { shadow_table_replace_block(map, app_blk_base); shadow_start = shadow_table_app_to_shadow(map, start); } memset(shadow_start, value, size); shdw_size += size; });
    *shadow_size = shdw_size;
    return DRMF_SUCCESS;
}

drmf_status_t umbra_shadow_copy_range_arch(IN umbra_map_t *map,
        IN app_pc app_src, IN app_pc app_dst, IN size_t app_size_in,
    ...
    APP_RANGE_LOOP(app_src, app_sz, app_blk_base, app_blk_end, app_src_end,
            start, end, iter_size,
            { shadow_start = shadow_table_app_to_shadow(map, start); if (shadow_table_is_in_default_block(map, shadow_start, NULL)) { res = DRMF_ERROR_INVALID_PARAMETER; break; } shadow_sz = umbra_map_scale_app_to_shadow(map, iter_size); res = umbra_write_shadow_memory_arch(map, app_dst, iter_size, &shadow_sz, shadow_start); if (res != DRMF_SUCCESS) { tot_shadow_sz += shadow_sz; break; } else { ASSERT(shadow_sz == umbra_map_scale_app_to_shadow(map, iter_size), "copy size mismatch"); } app_dst += iter_size; tot_shadow_sz += shadow_sz; });
    if (overlap_tail != NULL) {
        if (res == DRMF_SUCCESS) {
            shadow_sz = umbra_map_scale_app_to_shadow(map, tail_size);
            res = umbra_write_shadow_memory_arch(map,
                    app_dst + (app_dst - app_src), tail_size, &shadow_sz,
                    overlap_tail);
...
drmf_status_t umbra_value_in_shadow_memory_arch(IN umbra_map_t *map,
        INOUT app_pc *app_addr, IN size_t app_size, IN ptr_uint_t value,
...
drmf_status_t umbra_insert_app_to_shadow_arch(void *drcontext, umbra_map_t *map,
        instrlist_t *ilist, instr_t *where, reg_id_t addr_reg,
        reg_id_t *scratch_regs, int num_scratch_regs) {
...
drmf_status_t umbra_iterate_shadow_memory_arch(umbra_map_t *map,
        void *user_data, shadow_iterate_func_t iter_func) {
    uint i;
...
drmf_status_t umbra_get_shadow_memory_type_arch(umbra_map_t *map,
        byte *shadow_addr, umbra_shadow_memory_type_t *shadow_type) {
    uint i;
    bool redzone;
...
drmf_status_t umbra_get_shadow_memory_arch(umbra_map_t *map, app_pc app_addr,
        byte **shadow_addr, umbra_shadow_memory_info_t *shadow_info) {
    if (shadow_addr != NULL)
    ...
    if (shadow_info != NULL) {
        ...
        shadow_info->shadow_base = shadow_table_app_to_shadow(map,
                shadow_info->app_base);
...
bool umbra_address_is_app_memory(app_pc pc) {
...
drmf_status_t umbra_replace_shared_shadow_memory_arch(umbra_map_t *map,
        app_pc app_addr, byte **shadow_addr) {
    shadow_table_replace_block(map, app_addr);
    *shadow_addr = shadow_table_app_to_shadow(map, app_addr);
    return DRMF_SUCCESS;
}

drmf_status_t umbra_create_shared_shadow_block_arch(IN umbra_map_t *map,
        IN ptr_uint_t value, IN size_t value_size, OUT byte **block) {
...
drmf_status_t umbra_get_shared_shadow_block_arch(IN umbra_map_t *map,
        IN ptr_uint_t value, IN size_t value_size, OUT byte **block) {
    uint i;
    for (i = 0; i < map->num_special_blocks; i++) {
        if (map->special_blocks[i].value == value
...
bool umbra_handle_fault(void *drcontext, byte *target, dr_mcontext_t *raw_mc,
        dr_mcontext_t *mc) {
    return false;
}
